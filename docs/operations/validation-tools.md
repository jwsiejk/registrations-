...
- **What it validates:**
  - Required Docker files (compose, env files, helper scripts, mutation runner, and init/bootstrap scripts)
  - Required init directories (`infra/docker/init/crm`, `infra/docker/init/erp`, `infra/docker/init/warehouse`)
  - Required source/bootstrap SQL directories (`db/crm/schema`, `db/crm/seed`, `db/crm/mutate`, `db/erp/schema`, `db/erp/seed`, `db/erp/mutate`, `db/warehouse/bootstrap`)
  - Each required source/bootstrap SQL directory is populated with at least one `.sql` file
  - Root `.env` exists
